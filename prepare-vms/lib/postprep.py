...
system(
    "if mountpoint -q /mnt; then sudo mkdir -p /mnt/docker && sudo ln -sfn /mnt/docker /var/lib/docker; fi"
)
...
system(
    """sudo -u docker tee -a /home/docker/.bashrc <<SQRL
...
# Custom .vimrc
system(
    """sudo -u docker tee /home/docker/.vimrc <<SQRL
syntax on
set autoindent
set expandtab
set number
...
system(
    """sudo tee /etc/sudoers.d/docker <<SQRL
...
system(
    "sudo add-apt-repository 'deb [arch=amd64] https://download.docker.com/linux/ubuntu xenial {}'".format(
        ENGINE_VERSION
    )
)
system("sudo apt-get -q update")
...
system(
    "sudo curl -sSL -o /usr/local/bin/docker-compose https://github.com/docker/compose/releases/download/{}/docker-compose-{}-{}".format(
        COMPOSE_VERSION, platform.system(), platform.machine()
    )
)
...
system(
    "sudo curl -sSL -o /usr/local/bin/docker-machine https://github.com/docker/machine/releases/download/v{}/docker-machine-{}-{}".format(
        MACHINE_VERSION, platform.system(), platform.machine()
    )
)
...
system(
    "sudo apt-get -qy install python-setuptools pssh apache2-utils httping htop unzip mosh tree"
)
...
while addresses:
    ...
    for ipaddr, name in zip(cluster, names):
        system(
            "grep ^{} /etc/hosts || echo {} {} | sudo tee -a /etc/hosts".format(
                ipaddr, ipaddr, name
            )
        )
    print(cluster)
    ...
    if ipv4 == cluster[0]:
        ...
        system(
            "sudo -u docker [ -f /home/docker/.ssh/id_rsa ] || sudo -u docker ssh-keygen -t rsa -f /home/docker/.ssh/id_rsa -P ''"
        )
    else:
